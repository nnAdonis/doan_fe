    ...
    return (
        <section className="mt-12 border-t pt-4">
            ...
            <div className="overflow-x-auto flex gap-4 py-2">
                {items.map(item => (
                    <Link key={item.link} to={`/detail?link=${encodeURIComponent(item.link)}`} className="shrink-0 w-[300px] bg-white rounded shadow hover:shadow-md">
                        <div className="h-40 overflow-hidden">
                            <img src={item.image || "https://via.placeholder.com/300x160"} className="w-full h-full object-cover"/>
                        </div>
                        <div className="p-3">
